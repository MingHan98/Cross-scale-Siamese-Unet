...
# dir_image0 = r'C:/Users/Administrator/Desktop/reg_tutorials/code_data/original_versions/data/train/mr_images'
# dir_image1 = r'C:/Users/Administrator/Desktop/reg_tutorials/code_data/original_versions/data/train/us_images'
# dir_label0 = r'C:/Users/Administrator/Desktop/reg_tutorials/code_data/original_versions/data/train/mr_labels'
# dir_label1 = r'C:/Users/Administrator/Desktop/reg_tutorials/code_data/original_versions/data/train/us_labels'
...
#
        List = list(self.file_objects[0].shape[0:3])
    ...
    def get_data_moving_jdm(self, case_indices=None, label_indices=None):
        ...
        if label_indices is None:  # e.g. images only
            data = [np.flip(np.transpose(np.asarray(self.file_objects[i].dataobj), [0, 2, 1]), 1) for i in case_indices]
        else:
            if len(label_indices) == 1:
                label_indices *= self.num_data
            data = [np.flip(np.transpose(self.file_objects[i].dataobj[..., j], [0, 2, 1]), 1) if self.num_labels[i] > 1
                    else np.flip(np.transpose(np.asarray(self.file_objects[i].dataobj), [0, 2, 1]), 1)
                    for (i, j) in zip(case_indices, label_indices)]
        return np.expand_dims(np.stack(data, axis=0), axis=4)


    def get_data_moving(self, case_indices=None, label_indices=None):
        if case_indices is None:
            case_indices = range(self.num_data)
        # todo: check the supplied label_indices smaller than num_labels
        if label_indices is None:  # e.g. images only
    ...
    def get_data_fixed(self, case_indices=None, label_indices=None):
        if case_indices is None:
            case_indices = range(self.num_data)
        # todo: check the supplied label_indices smaller than num_labels
        if label_indices is None:  # e.g. images only
...
def write_images(input_, file_path=None, file_prefix=''):
    if file_path is not None:
        ...
        affine = [[-2.4, 0, 0, 0], [0, -2.4, 0, 239], [0, 0, 2.4, 0], [0, 0, 0, 1]]
...
class ConfigParser:
    def __init__(self, argv='', config_type='all'):
        ...
        if os.path.isfile(filename_):
            ...
            self.config_file.read(filename_, encoding='UTF-8')
        else:
    ...
    def get_defaults(self):
        ...
        home_dir = r'/data/smh/data/jdm_data/primary/cross_1/'

        network = {'network_type': 'smeunet'}

        data = {'dir_moving_image': os.path.join(home_dir, 'train/demo_data/mr_images'),
                'dir_fixed_image': os.path.join(home_dir, 'train/demo_data/us_images'),
                'dir_moving_label': os.path.join(home_dir, 'train/demo_data/mr_labels'),
                'dir_fixed_label': os.path.join(home_dir, 'train/demo_data/us_labels')}

        loss = {'similarity_type': 'dice',
                'similarity_scales': [0, 1, 2, 4, 8],
        ...
        inference = {'file_model_saved': train['file_model_save'],
                     'dir_moving_image': os.path.join(home_dir, 'test/demo_data/mr_images'),
                     'dir_fixed_image': os.path.join(home_dir, 'test/demo_data/us_images'),
