...
def spp_layer(input_, levels=4, name='SPP_layer', pool_type='max_pool'):
    ...
    with tf.variable_scope(name):
        ...
        for l in range(levels):
            ...
            ksize = [1, np.ceil(2**l).astype(np.int32), np.ceil(2**l).astype(np.int32), 1]

            # strides = [1, np.floor(shape[1] / l + 1).astype(np.int32), np.floor(shape[2] / l + 1).astype(np.int32), 1]
            strides = [1, np.floor(2**l).astype(np.int32), np.floor(2**l).astype(np.int32), 1]
...
data = tf.random_normal(shape=[2, 28, 16, 64], name='data_ran')
a = spp_layer(data)
print('ok')
